...
# def visualize_single_sample(writer, model, val_loader, epoch):
#     """Plot activations, prediction, GT, and Grad-CAM as separate full-size figures"""    
#     # Get sample
#     torch.manual_seed(epoch)  # Makes selection consistent per epoch
#     inputs, targets, _ = next(iter(val_loader))
#     idx = torch.randint(0, inputs.size(0), (1,)).item()
...
#     # Regular forward pass for metrics
#     with torch.no_grad():
#         if isinstance(model, torch.nn.DataParallel):
#             output, activations = model.module(input_img)
#         else:
...
#     # Use output as target for gradient computation (for segmentation)
#     # We want to see what contributes to the positive predictions
#     target = output
#     target = target.mean()  # Average across spatial dimensions
...
#     if isinstance(model, torch.nn.DataParallel):
#         gradients = model.module.get_activations_gradient()  # You'll need to implement this hook
#     else:
#         gradients = model.get_activations_gradient()  # You'll need to implement this hook
...
#     grad_cam = torch.mean(weighted_activations, dim=1).squeeze()  # Average across channels
...
#     target_mask = target_mask.cpu().squeeze()
#     output = output_sigmoid.cpu().squeeze()
#     activations = activations.detach().cpu().squeeze()
#     grad_cam = grad_cam.detach().cpu()
    
#     # Normalize activations (mean across channels if needed)
#     if len(activations.shape) == 3:
#         activations_mean = activations.mean(dim=0)
#     activations_mean = (activations_mean - activations_mean.min()) / (activations_mean.max() - activations_mean.min() + 1e-6)

#     # -----------------------------------------------
#     # 1. Activation Heatmap (Full Page)
#     plt.figure(figsize=(12, 10))
#     plt.imshow(activations_mean, cmap='jet', vmin=0, vmax=1)
#     plt.title(f"Activation Map (PIoU: {piou:.2f})", fontsize=16, pad=20)
#     plt.axis('off')
#     cbar = plt.colorbar(fraction=0.046, pad=0.04)
#     cbar.ax.tick_params(labelsize=12)
...
#     plt.figure(figsize=(12, 10))
#     plt.imshow(input_img, cmap='gray')  # Original image
#     plt.imshow(grad_cam, cmap='jet', alpha=0.5)  # Grad-CAM overlay
