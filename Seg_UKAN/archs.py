...
class UKAN(nn.Module):
    def __init__(self, num_classes=1, input_channels=3, deep_supervision=False, 
        ...
        if deep_supervision:
            self.ds1 = nn.Conv2d(embed_dims[1], num_classes, kernel_size=1)
            self.ds2 = nn.Conv2d(embed_dims[0], num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Decoder
        x = F.relu(F.interpolate(self.decoder1(x), scale_factor=2, mode='bilinear'))
        x = torch.add(x, t4)
        
        if self.deep_supervision:
            ds1 = F.interpolate(self.ds1(x), scale_factor=8, mode='bilinear')

        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)
        x = self.dnorm3(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x = F.relu(F.interpolate(self.decoder2(x), scale_factor=2, mode='bilinear'))
        x = torch.add(x, e3)
        
        if self.deep_supervision:
            ds2 = F.interpolate(self.ds2(x), scale_factor=4, mode='bilinear')

        x = F.relu(F.interpolate(self.decoder3(x), scale_factor=2, mode='bilinear'))
        x = torch.add(x, e2)
        x = F.relu(F.interpolate(self.decoder4(x), scale_factor=2, mode='bilinear'))
        x = torch.add(x, e1)
        x = F.relu(F.interpolate(self.decoder5(x), scale_factor=2, mode='bilinear'))
        
        final_out = self.final(x)
        
        if self.deep_supervision:
            return [ds1, ds2, final_out]
        return final_out