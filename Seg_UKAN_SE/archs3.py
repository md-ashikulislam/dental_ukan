...
class ECABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.conv(y.transpose(-1, 1)).transpose(-1, 1).view(b, c, 1, 1)
        y = self.sigmoid(y)
...
class KANLayer(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., no_kan=False):
        ...
        self.eca1 = ECABlock(hidden_features)
        self.eca2 = ECABlock(hidden_features)
        self.eca3 = ECABlock(hidden_features)
    ...
    def forward(self, x, H, W):
        ...
        x = self.eca1(x.reshape(B, C, H, W)).reshape(B, N, C)
        x = self.dwconv_1(x, H, W)
        
        x = self.fc2(x.reshape(B*N,C))
        x = x.reshape(B,N,C).contiguous()
        x = self.eca2(x.reshape(B, C, H, W)).reshape(B, N, C)
        x = self.dwconv_2(x, H, W)
        
        x = self.fc3(x.reshape(B*N,C))
        x = x.reshape(B,N,C).contiguous()
        x = self.eca3(x.reshape(B, C, H, W)).reshape(B, N, C)
...
class UKAN_ECA(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3, 
        ...
        self.gate1 = GatedFusion(embed_dims[1])
        self.gate2 = GatedFusion(embed_dims[0])
        self.gate3 = GatedFusion(embed_dims[0]//4)
        self.gate4 = GatedFusion(embed_dims[0]//8)
    ...
    def forward(self, x):
        ...
        out = self.gate3(out, t2)
        out = F.relu(F.interpolate(self.decoder4(out), scale_factor=(2,2), mode='bilinear'))
        out = self.gate4(out, t1)
